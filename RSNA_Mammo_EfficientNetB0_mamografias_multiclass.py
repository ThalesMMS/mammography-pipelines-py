...
# -*- coding: utf-8 -*-
"""
RSNA_Mammo_EfficientNetB0_Density_Multiclass.py
----------------------------------
Treinamento/inferência com EfficientNetB0 para classificação multiclasse de densidade mamária:
  - Classe 0: BI-RADS 1 (A - baixa densidade)
  - Classe 1: BI-RADS 2 (B - baixa densidade)
  - Classe 2: BI-RADS 3 (C - alta densidade)
  - Classe 3: BI-RADS 4 (D - alta densidade)
Mapeia automaticamente labels originais (1,2,3,4) para (0,1,2,3) e extrai embeddings (1280-D) a partir de DICOMs.
...
Uso rápido:
    python RSNA_Mammo_EfficientNetB0_mamografias_multiclass.py `
    --csv mamografias `
    --epochs 20 `
    --batch-size 16 `
    --img-size 512 `
    --lr 1e-4 `
    --backbone-lr 1e-5 `
    --outdir outputs/mammo_efficientnetb0_mamografias_multiclass `
    --device auto `
    --amp `
    --unfreeze-last-block `
    --class-weights auto `
...
class MammoDensityDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        """Retorna `(tensor_normalizado, label_multiclasse, metadados)` e materializa o cache sob demanda quando ausente.
        
        Labels são mapeados de 1..4 (A,B,C,D) para 0..3 (multiclasse):
        - 1 (BI-RADS 1, A) → 0
        - 2 (BI-RADS 2, B) → 1
        - 3 (BI-RADS 3, C) → 2
        - 4 (BI-RADS 4, D) → 3
        ...
        # map 1..4 -> 0..3 (multiclasse: A, B, C, D)
        if y is not None and y in {1, 2, 3, 4}:
            y = int(y) - 1  # 1->0, 2->1, 3->2, 4->3
        else:
            y = None
        return img, y, r
...
def make_splits(df: pd.DataFrame, val_frac: float = 0.2, seed: int = 42) -> Tuple[List[int], List[int]]:
    """Split estratificado por rótulo multiclasse (0=A, 1=B, 2=C, 3=D) agrupado por accession para evitar vazamento.
    
    Mapeia labels originais 1..4 para multiclasse 0..3 antes do split estratificado.
    ...
    Se faltar alguma classe, cai para split aleatório agrupado.
    """
    rng = np.random.RandomState(seed)
    y_orig = df["professional_label"].values
    y_orig = np.array([l for l in y_orig])
    idx = np.arange(len(df))
    # filtra apenas rótulos 1..4 e mapeia para multiclasse 0..3
    ...
    for i, orig_label in zip(idx_candidates, y_orig[mask]):
        multiclass_label = int(orig_label) - 1  # 1->0, 2->1, 3->2, 4->3
        if multiclass_label in {0, 1, 2, 3}:
            y_multiclass_list.append(multiclass_label)
            idx_valid.append(i)
            # Usar accession como grupo para evitar vazamento
            groups_list.append(df.iloc[i]["accession"])
    y_labeled_multiclass = np.array(y_multiclass_list)
    idx_labeled = np.array(idx_valid)
    groups = np.array(groups_list)
    
    if len(np.unique(y_labeled_multiclass)) < 2:
    ...
    for tr, va in gss.split(idx_labeled, y_labeled_multiclass, groups=groups):
...
def build_model(num_classes: int = 4, train_backbone: bool = False, unfreeze_last_block: bool = True) -> nn.Module:
    """Cria EfficientNetB0 pré-treinada, troca o 'classifier' por uma cabeça de 4 saídas (multiclasse: A, B, C, D).
...
def validate(model, loader, device, *, amp_enabled: bool = False) -> Dict[str, Any]:
    """Validação multiclasse: acumula predições e probabilidades, calcula métricas e retorna linhas detalhadas.
    
    Labels multiclasse: 0=A (BI-RADS 1), 1=B (BI-RADS 2), 2=C (BI-RADS 3), 3=D (BI-RADS 4).
    ...
    # métricas multiclasse (0=A, 1=B, 2=C, 3=D)
    acc = accuracy_score(y_true, y_pred)
    kappa = cohen_kappa_score(y_true, y_pred)
    try:
        # AUC multiclasse usando one-vs-rest (ovr)
        auc = roc_auc_score(y_true, prob, multi_class='ovr', average='macro')
    except Exception:
        auc = None
    cm = confusion_matrix(y_true, y_pred, labels=[0, 1, 2, 3]).tolist()
    report = classification_report(y_true, y_pred, labels=[0, 1, 2, 3], output_dict=True, zero_division=0, target_names=["A", "B", "C", "D"])
    ...
    for i, m in enumerate(rows):
        ...
        out_rows.append({
            **m,
            "y_true": int(y_true[i]),
            "y_pred": int(y_pred[i]),
            "y_true_label": class_names[y_true[i]] if 0 <= y_true[i] < 4 else "Unknown",
            "y_pred_label": class_names[y_pred[i]] if 0 <= y_pred[i] < 4 else "Unknown",
            "p_A": float(pr[0]),
            "p_B": float(pr[1]),
            "p_C": float(pr[2]),
            "p_D": float(pr[3]),
...
def save_metrics_figure(metrics: Dict[str, Any], out_path: str) -> None:
    """Gera uma figura com matriz de confusão e barras de precisão/recall/F1 por classe (multiclasse).
    - metrics: dicionário retornado por validate()
    - out_path: caminho do PNG de saída (ex.: .../metrics/val_metrics.png)
    """
    try:
        cm = np.array(metrics.get("confusion_matrix", [[0]*4]*4), dtype=float)
        report = metrics.get("classification_report", {})
        acc = float(metrics.get("acc", 0.0))
        kappa = float(metrics.get("kappa", 0.0))
        auc = metrics.get("auc", None)

        labels = ["A", "B", "C", "D"]  # multiclasse: BI-RADS 1, 2, 3, 4
        cls_names = ["A", "B", "C", "D"]
...
def save_history_figure(hist_rows: List[Dict[str, Any]], out_path: str) -> None:
    ...
    try:
        ...
        if "val_auc" in dfh.columns:
            try:
                ax.plot(dfh["epoch"], dfh["val_auc"], label="auc (ovr)", color="#9467bd")
            except Exception:
                pass
        elif "val_auc_ovr" in dfh.columns:
            try:
                ax.plot(dfh["epoch"], dfh["val_auc_ovr"], label="auc (ovr)", color="#9467bd")
            except Exception:
                pass
        ax.set_xlabel("Época")
...
def run(args: argparse.Namespace):
    ...
    # 2) Filtra labels 1..4 e prepara para mapeamento multiclasse
    df = df.copy()
    ...
    # Log distribuição original e multiclasse
    dist_orig = df["professional_label"].value_counts().sort_index().to_dict()
    df_multiclass_temp = df.copy()
    df_multiclass_temp["multiclass_label"] = df_multiclass_temp["professional_label"].apply(lambda x: int(x) - 1 if x in {1,2,3,4} else None)
    dist_multiclass = df_multiclass_temp["multiclass_label"].value_counts().sort_index().to_dict()
    logger.info("Distribuição de rótulos originais (1-4): %s", dist_orig)
    logger.info("Distribuição de rótulos multiclasse (0=A, 1=B, 2=C, 3=D): %s", dist_multiclass)
    ...
    if args.sampler_weighted:
        # constrói pesos por amostra com base na frequência de classe multiclasse no treino (0=A, 1=B, 2=C, 3=D)
        ys = [int(r.get("professional_label")) - 1 if r.get("professional_label") in {1,2,3,4} else None for r in rows_tr]
        ys_multiclass = [y for y in ys if y is not None]
        counts = {c: max(1, ys_multiclass.count(c)) for c in range(4)}
        import torch.utils.data as tud
        sample_weights = [1.0 / counts[y] for y in ys_multiclass]
        ...
        LOGGER.info("Treinando com WeightedRandomSampler (classes multiclasse balanceadas: A, B, C, D).")
    else:
    ...
    model = build_model(num_classes=4, train_backbone=args.train_backbone, unfreeze_last_block=args.unfreeze_last_block)
    model.to(device)
    compiled = False
    ...
    # 5.1) Loss ponderada opcional: 'auto' usa pesos ~ inverso da frequência por classe multiclasse no treino
    loss_fn = nn.CrossEntropyLoss()
    if args.class_weights == "auto":
        ys = [int(r.get("professional_label")) - 1 if r.get("professional_label") in {1,2,3,4} else None for r in rows_tr]
        ys_multiclass = [y for y in ys if y is not None]
        counts = np.array([max(1, ys_multiclass.count(c)) for c in range(4)], dtype=np.float64)
        if np.any(counts == 0):
            LOGGER.warning("Alguma classe multiclasse ausente no treino; ignorando class weights.")
        else:
            # Regra simples: weight_c = N_total / (n_classes * count_c)
            weights = (len(ys_multiclass) / (4.0 * counts)).astype(np.float32)
            cw = torch.tensor(weights, dtype=torch.float32, device=device)
            loss_fn = nn.CrossEntropyLoss(weight=cw)
            LOGGER.info("Class weights (auto, multiclasse): A=%s, B=%s, C=%s, D=%s", weights[0], weights[1], weights[2], weights[3])
    ...
    with context:
        for epoch in range(1, args.epochs + 1):
            ...
            hist_rows.append({
                ...
                "val_auc_ovr": (val["auc"] if val["auc"] is not None else np.nan),  # alias para compatibilidade
                "sec": round(t1 - t0, 2),
            })
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    """
    p = argparse.ArgumentParser(description="EfficientNetB0 - Classificação multiclasse de densidade mamária (A, B, C, D) com Transfer Learning")
    ...
    p.add_argument("--outdir", default="outputs/mammo_efficientnetb0_density_multiclass", help="Pasta raiz para results_*/metrics e cache compartilhado")
