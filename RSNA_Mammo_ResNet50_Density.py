...
# -*- coding: utf-8 -*-
"""
RSNA_Mammo_ResNet50_Density.py
----------------------------------
Treinamento/inferência com ResNet50 para classificação de densidade mamária (4 classes),
comparação com rótulo profissional e extração de embeddings (2048-D) a partir de DICOMs.
...
LOGGER = logging.getLogger("mammo_resnet50_density")
...
class HP:
    ...
    - LR / BACKBONE_LR: taxas de aprendizado (head e layer4/backbone).
    ...
    LR: float = 1e-4 # taxas de aprendizado (head e layer4/backbone)
    BACKBONE_LR: float = 1e-5 # taxas de aprendizado (head e layer4/backbone)
    ...
    UNFREEZE_LAST_BLOCK: bool = True # padrão: descongela layer4 (último bloco); use --no-unfreeze-last-block para congelar
...
def _increment_path(path: str) -> str:
    ...
    Ex.: "outputs/mammo_resnet50_density" → "outputs/mammo_resnet50_density_1" se a primeira existir.
    """
    base = path.rstrip("/\\")
...
def dicom_to_pil_rgb(dcm_path: str) -> Image.Image:
    ...
    Mantemos 3 canais replicados para compatibilidade com ResNet50 pré-treinada no ImageNet.
    """
    try:
...
class MammoDensityDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        """Retorna `(tensor_normalizado, label_0_3, metadados)` e materializa o cache sob demanda quando ausente."""
        r = self.rows[i]
...
def build_model(num_classes: int = 4, train_backbone: bool = False, unfreeze_last_block: bool = True) -> nn.Module:
    """Cria ResNet50 pré-treinada, troca a 'fc' por uma cabeça de 4 saídas.
    
    Por padrão: congela layers iniciais (conv1, bn1, layer1, layer2, layer3) e descongela layer4 + fc.
    - unfreeze_last_block=True (padrão): descongela layer4 (último bloco)
    - unfreeze_last_block=False: congela layer4 também (apenas fc treina)
    - train_backbone=True: descongela todo o backbone (não recomendado; apenas para experimentação)
    """
    m = resnet50(weights=ResNet50_Weights.IMAGENET1K_V2)
    in_features = m.fc.in_features
    m.fc = nn.Linear(in_features, num_classes) # troca a 'fc' por uma cabeça de 4 saídas
    
    # congela todas as camadas por padrão
    for p in m.parameters():
        p.requires_grad = False
    
    # sempre treina a head nova (fc)
    for p in m.fc.parameters():
        p.requires_grad = True
    
    # por padrão, descongela layer4 (últimas camadas)
...
def extract_embeddings(
    ...
    *,
    ...
    """Extrai features 2048-D da 'avgpool' (penúltima camada) via forward-hook.
...
def run(args: argparse.Namespace):
    ...
    # 1) Carrega CSV (classificacao.csv ou baseado em paths)
    ...
    # 2) Filtra labels 1..4 e mapeia para 0..3
    df = df.copy()
    ...
    logger.debug("Distribuição de rótulos (1-4): %s", df["professional_label"].value_counts().sort_index().to_dict())
    ...
    if args.sampler_weighted:
        # constrói pesos por amostra com base na frequência de classe no treino (0..3)
        ys = [r.get("professional_label") for r in rows_tr]
    ...
    # 5.1) Loss ponderada opcional: 'auto' usa pesos ~ inverso da frequência por classe no treino
    loss_fn = nn.CrossEntropyLoss()
    if args.class_weights == "auto":
        ys = [int(r.get("professional_label")) - 1 for r in rows_tr]
    ...
    # Warmup opcional: se warmup_epochs > 0, congela layer4 temporariamente e treina só a head nas primeiras épocas
    if args.warmup_epochs > 0 and not args.train_backbone and args.unfreeze_last_block:
        for p in model.layer4.parameters():
            p.requires_grad = False
        LOGGER.info("Warmup ativo: treinar apenas a head por %d época(s); layer4 será liberado após warmup", args.warmup_epochs)
    ...
    if any(p.requires_grad for p in layer4_params):
        param_groups.append({"params": [p for p in layer4_params if p.requires_grad], "lr": args.backbone_lr})
    ...
    with context:
        for epoch in range(1, args.epochs + 1):
            # Warmup: após warmup_epochs, libera layer4 (se estava congelado pelo warmup)
            if args.warmup_epochs > 0 and epoch == args.warmup_epochs + 1 and not args.train_backbone and args.unfreeze_last_block:
                if not any(p.requires_grad for p in model.layer4.parameters()):
                    for p in model.layer4.parameters():
                        p.requires_grad = True
                    param_groups = [
                        {"params": [p for p in model.fc.parameters() if p.requires_grad], "lr": args.lr},
                        {"params": [p for p in model.layer4.parameters() if p.requires_grad], "lr": args.backbone_lr},
                    ]
                    optim = torch.optim.AdamW(param_groups, weight_decay=args.weight_decay)
                    LOGGER.info("Warmup concluído: liberando layer4 no epoch %d com backbone_lr=%s", epoch, args.backbone_lr)
            t0 = time.time()
    ...
    # salva CSV combinando metadados + embeddings (como colunas e0..e2047)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    """
    p = argparse.ArgumentParser(description="ResNet50 - Densidade mamária com Transfer Learning")
    ...
    p.add_argument("--outdir", default="outputs/mammo_resnet50_density", help="Pasta raiz para results_*/metrics e cache compartilhado")
    ...
    p.add_argument("--backbone-lr", dest="backbone_lr", type=float, default=HP.BACKBONE_LR, help="LR para layer4/backbone quando destravado")
    ...
    p.add_argument("--unfreeze-last-block", dest="unfreeze_last_block", action="store_true", help="Descongela layer4 (padrão)")
    p.add_argument("--no-unfreeze-last-block", dest="unfreeze_last_block", action="store_false", help="Congela layer4 (apenas fc treina)")
    ...
    p.add_argument("--warmup-epochs", dest="warmup_epochs", type=int, default=HP.WARMUP_EPOCHS, help="Épocas só na head antes de liberar layer4 (layer4 precisa estar descongelado via --unfreeze-last-block)")
