...
# -*- coding: utf-8 -*-
"""
RSNA_Mammo_ResNet50_Density_ABxCD.py
----------------------------------
Treinamento/inferência com ResNet50 para classificação binária de densidade mamária:
  - Classe 0: A/B (baixa densidade, BI-RADS 1-2)
  - Classe 1: C/D (alta densidade, BI-RADS 3-4)
Mapeia automaticamente labels originais (1,2,3,4) para binário (0,1) e extrai embeddings (2048-D) a partir de DICOMs.
...
Uso rápido:
  python RSNA_Mammo_ResNet50_Density_ABxCD.py \
    --csv classificacao.csv \
    --dicom-root archive \
    --epochs 8 --batch-size 16 --img-size 512 \
    --outdir outputs/mammo_resnet50_density_abxcd
...
def make_splits(df: pd.DataFrame, val_frac: float = 0.2, seed: int = 42) -> Tuple[List[int], List[int]]:
    """Split estratificado por rótulo binário (0=AB, 1=CD).
    
    Mapeia labels originais 1..4 para binário 0..1 antes do split estratificado.
    Se faltar alguma classe binária, cai para split aleatório.
    ...
    for tr, va in skf.split(idx_labeled, y_labeled_binary):
...
def build_model(num_classes: int = 2, train_backbone: bool = False, unfreeze_last_block: bool = True) -> nn.Module:
    """Cria ResNet50 pré-treinada, troca a 'fc' por uma cabeça de 2 saídas (binário: AB vs CD).
    ...
    m.fc = nn.Linear(in_features, num_classes) # troca a 'fc' por uma cabeça de 2 saídas (binário)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    """
    p = argparse.ArgumentParser(description="ResNet50 - Classificação binária de densidade mamária (AB vs CD) com Transfer Learning")
    ...
    p.add_argument("--outdir", default="outputs/mammo_resnet50_density_abxcd", help="Pasta raiz para results_*/metrics e cache compartilhado")
