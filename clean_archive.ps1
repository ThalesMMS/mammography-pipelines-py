...
foreach ($dir in $subdirs) {
    Write-Host "`nProcessando: $($dir.Name)" -ForegroundColor Yellow
    
    # Remove todos os arquivos PNG
    $pngFiles = Get-ChildItem -Path $dir.FullName -Filter "*.png"
    if ($pngFiles.Count -gt 0) {
        Write-Host "  Removendo $($pngFiles.Count) arquivo(s) PNG..." -ForegroundColor Gray
        $pngFiles | Remove-Item -Force
    }
    
    # Processa arquivos DICOM
    $dcmFiles = Get-ChildItem -Path $dir.FullName -Filter "*.dcm" | Sort-Object Name
