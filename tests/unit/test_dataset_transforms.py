...
legacy_transforms = pytest.importorskip("torchvision.transforms")
InterpolationMode = pytest.importorskip("torchvision.transforms").InterpolationMode
...
def test_dataset_matches_legacy_pipeline(tmp_path, train):
    ...
    new_img, new_label, _, _ = dataset[0]
...
def test_transforms_run_equally_on_gpu(tmp_path):
    ...
    if device.type == "mps":
        pytest.skip("MPS backend does not guarantee deterministic augmentation parity with CPU.")
