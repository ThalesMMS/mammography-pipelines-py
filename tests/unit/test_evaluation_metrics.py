...
# from mammography.eval.clustering_metrics import ClusteringMetrics
# from mammography.eval.silhouette_evaluator import SilhouetteEvaluator
# from mammography.eval.davies_bouldin_evaluator import DaviesBouldinEvaluator
...
class TestEvaluationMetrics:
    ...
    def test_silhouette_score_different_clusterings(self, sample_data_with_clusters):
        ...
        for clustering in clusterings:
            n_labels = len(set(clustering))
            if 1 < n_labels < len(clustering):
    ...
    def test_evaluation_metrics_comparison(self, sample_data_with_clusters):
        ...
        for name, clustering in clusterings:
            n_labels = len(set(clustering))
            if n_labels > 1:  # Need at least 2 clusters for most metrics
                metrics = {}

                # Calculate metrics that require 2..n_samples-1 clusters.
                if 1 < n_labels < len(clustering):
    ...
    def test_evaluation_metrics_with_different_data_sizes(
        ...
        for size in data_sizes:
            ...
            n_labels = len(set(subset_labels))

            # Calculate metrics (guard against invalid cluster counts).
            if 1 < n_labels < len(subset_labels):
