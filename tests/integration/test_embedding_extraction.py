...
# from mammography.models.embeddings.resnet_extractor import ResNetEmbeddingExtractor
# from mammography.models.embeddings.input_adapter import InputAdapter
# from mammography.models.embeddings.feature_pooler import FeaturePooler
    ...
    def test_embedding_dimensions(self, sample_tensors, resnet50_model):
        ...
        for config in model_configs:
            if config["name"] == "resnet50":
                ...
                try:
                    model = getattr(models, config["name"])(weights=None)
                except TypeError:
                    model = getattr(models, config["name"])(pretrained=False)
                model.eval()
