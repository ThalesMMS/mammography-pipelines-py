...
# from mammography.preprocess.normalizer import Normalizer
        ...
        for config in invalid_configs:
            with pytest.raises((ValueError, KeyError)):
                ...
                if "target_size" in config:
                    if not all(x > 0 for x in config["target_size"]):
                        raise ValueError("Invalid target size")
                if "normalization_method" in config:
                    if config["normalization_method"] not in [
                        "z_score_per_image",
                        "fixed_window",
                    ]:
                        raise ValueError("Invalid normalization method")
                if "input_adapter" in config:
                    if config["input_adapter"] not in [
                        "1to3_replication",
                        "conv1_adapted",
                    ]:
                        raise ValueError("Invalid input adapter")
