...
> Research software only. Not for clinical use, diagnosis, or patient-care decisions.

## Release status

- Package metadata currently marks the project as **Alpha**.
- Treat GitHub releases as **research snapshots / prereleases** until the stable-release gates in [`docs/release-checklist.md`](docs/release-checklist.md) are met.
- Release assets should contain source and build artifacts only, never patient data, private datasets, or trained weights that cannot be redistributed.
...
- [`CHANGELOG.md`](CHANGELOG.md) tracks human-maintained release history.
- [`docs/release-checklist.md`](docs/release-checklist.md) defines prerelease vs stable-release gates.
- `.github/workflows/release.yml` builds source, wheel, and `SHA256SUMS.txt` assets when a GitHub release is published.
- `.github/release.yml` organizes GitHub autogenerated release notes.

## Citation and minimum reproducibility bundle

- [`CITATION.cff`](CITATION.cff) provides software citation metadata for manuscripts, posters, and internal research reports.
- For any reported result, keep the exact CLI command or config file together with the workflow-specific artifacts already produced by this repository:
  - training runs: `summary.json`, `metrics/val_metrics.json`, `best_model.pt` (view-specific training appends view suffixes to artifact names, for example `best_model_cc.pt` and `best_model_mlo.pt`)
  - embedding runs: `features.npy`, `metadata.csv`, and `joined.csv` when exported
  - cross-validation runs: `cv_summary.json` plus each fold's `metrics.json`
  - data audits: `data_manifest.json` and the corresponding `data_audit.csv`
- Preserve the existing research-only framing when redistributing outputs, and do not publish patient data or non-redistributable weights.
