...
# Optional import: opencv-python (cv2) for advanced image processing
try:
    import cv2
    HAS_CV2 = True
except ImportError:
...
def _resize_array(
    ...
    """
    Resize array using cv2 if available, otherwise fall back to scipy.

    Args:
        array: Input array to resize
        target_size: Target (width, height) for cv2 or (height, width) for scipy
        interpolation: cv2 interpolation flag (ignored for scipy fallback)
    ...
    if HAS_CV2:
        ...
        return cv2.resize(array, target_size, interpolation=interpolation or cv2.INTER_LINEAR)
    else:
        # Scipy fallback - expects (height, width)
        # Note: target_size is (width, height) from cv2 convention, so swap
...
class ImagePreprocessor:
    ...
    SUPPORTED_NORMALIZATION_METHODS: ClassVar[List[str]] = [
        "z_score_per_image",
        "fixed_window",
        "min_max_scaling",
        "percentile_scaling",
    ...
    SUPPORTED_INPUT_ADAPTERS: ClassVar[List[str]] = ["1to3_replication", "conv1_adapted"]
    ...
    def _read_pixel_data(
        ...
        try:
            ...
            if hasattr(dataset, "PhotometricInterpretation") and dataset.PhotometricInterpretation == "MONOCHROME1":
    ...
    def _resize_image(self, pixel_array: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if keep_aspect_ratio:
                ...
                resized_array = _resize_array(
                    pixel_array,
                    (target_size[1], target_size[0]),
                    interpolation=cv2.INTER_LINEAR if HAS_CV2 else None,
                )
    ...
    def _resize_with_aspect_ratio(
        ...
        # Resize image
        resized_array = _resize_array(
            pixel_array, (new_w, new_h), interpolation=cv2.INTER_LINEAR if HAS_CV2 else None
        )
    ...
    def _normalize_to_uint8(self, pixel_array: np.ndarray) -> np.ndarray:
        ...
        # Normalize to [0, 255] range in a single operation to reduce allocations
        # Using multiply and clip to avoid intermediate arrays
        scale = 255.0 / (max_val - min_val)
        return np.clip((pixel_array - min_val) * scale, 0, 255).astype(np.uint8)
