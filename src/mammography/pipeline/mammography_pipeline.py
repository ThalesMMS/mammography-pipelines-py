...
class MammographyPipeline:
    ...
    def _validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Educational Note: Configuration validation ensures all required
        parameters are present and within valid ranges. Sets default values
        for optional parameters including max_workers (default: 4) for
    ...
    def _run_preprocessing_step(
        ...
        try:
            ...
            results = self._process_preprocessing_files_parallel(
                dicom_files, preprocessing_results["output_dir"]
            )
    ...
    def _process_single_preprocessing_file(
        ...
        """
        Process a single DICOM file for preprocessing.
        ...
        try:
            ...
            output_path = self._get_preprocessing_output_path(file_path, output_dir)
    ...
    def _process_preprocessing_files_parallel(
        self, dicom_files: List[Path], output_dir: Path
    ) -> List[Optional[Dict[str, Any]]]:
        """
        Process DICOM files in parallel for preprocessing.
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_index = {
                executor.submit(
                    self._process_single_preprocessing_file, file_path, output_dir
                ): i
    ...
    def _run_embedding_step(
        ...
        """
        Run embedding extraction step with parallel processing.
        ...
        try:
            ...
            results = self._process_embedding_files_parallel(
                tensor_files, embedding_results["output_dir"]
            )
    ...
    def _process_single_embedding_file(
        ...
        """
        Process a single tensor file for embedding extraction.
        ...
        try:
            ...
            # Save embedding
            output_path = self._get_embedding_output_path(file_path, output_dir)
    ...
    def _process_embedding_files_parallel(
        self, tensor_files: List[Path], output_dir: Path
    ) -> List[Optional[Dict[str, Any]]]:
        """
        Process tensor files in parallel for embedding extraction.

        Args:
            tensor_files: List of tensor file paths
            output_dir: Output directory for results
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_index = {
                executor.submit(
                    self._process_single_embedding_file, file_path, output_dir
                ): i
...
def run_mammography_pipeline(
    ...
    Args:
        input_dir: Directory containing DICOM files
        output_dir: Directory to save results
        config: Pipeline configuration dictionary. Can include max_workers
            parameter to control parallel processing (default: 4 workers).
        device: Device to use for processing
