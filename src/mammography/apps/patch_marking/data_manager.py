...
def _redact_values(values: list, label: str) -> list[str]:
    return [fingerprint_value(value, label) for value in values]
...
class DataManager:
    ...
    def __init__(self, archive_dir_path: str):
        ...
        self.navigable_folders = (
            []
        )  # Folders available for navigation (can be filtered)
        self.current_folder_index = -1

        if not os.path.isdir(self.archive_dir):
            raise FileNotFoundError(
                f"The 'archive' directory was not found at: {redact_path(self.archive_dir)}"
            )

        if not os.path.isfile(self.train_csv_path):
            raise FileNotFoundError(
                f"The 'train.csv' file was not found at: {redact_path(self.train_csv_path)}"
            )
    ...
    def _load_train_data(self):
        ...
        try:
            df = pd.read_csv(
                self.train_csv_path, dtype={"AccessionNumber": str, "PatientID": str}
            )
            ...
            print(
                f"train.csv loaded successfully. {len(self.patient_data_df)} records found."
            )
        except pd.errors.EmptyDataError:
            print(f"Error: The file {redact_path(self.train_csv_path)} is empty.")
            ...
            print(
                f"Unexpected error while loading {redact_path(self.train_csv_path)}: {e}"
            )
            self.patient_data_df = pd.DataFrame()
    ...
    def _scan_patient_folders(self):
        ...
        if not self._all_valid_patient_folders:
            ...
            print(
                f"Found {len(self._all_valid_patient_folders)} valid patient folders with labels."
            )
    ...
    def filter_folders(
        self, only_folders_without_pngs: bool, target_value_filter: int | None = None
    ):
        """
        ...
        if only_folders_without_pngs:
            ...
            print(
                f"- PNG filter: {len(candidate_folders)} folder(s) remaining after checking PNGs."
            )
        else:
        ...
        if target_value_filter is not None:
            if self.patient_data_df is None:
                print(
                    "Warning: Cannot filter by target because train.csv data is not loaded."
                )
            else:
                # Ensure the 'target' column exists and is numeric
                if (
                    "target" in self.patient_data_df.columns
                    and pd.api.types.is_numeric_dtype(self.patient_data_df["target"])
                ):
                    ...
                    candidate_folders = [
                        ...
                        if folder_name in self.patient_data_df.index
                        and int(self.patient_data_df.loc[folder_name, "target"])
                        == target_value_filter
                    ]
                    print(
                        f"- Target filter={target_value_filter}: {len(candidate_folders)} folder(s) remaining (from {original_count})."
                    )
                else:
                    print(
                        "Warning: 'target' column not found or not numeric in the DataFrame. Target filter ignored."
                    )
        ...
        if self.navigable_folders:
            self.current_folder_index = 0
            print(
                f"Total navigable folders after filtering: {len(self.navigable_folders)}"
            )
        else:
            self.current_folder_index = -1
    ...
    def get_patient_info_from_df(self, accession_number: str) -> pd.Series | None:
        ...
        """
        if (
            self.patient_data_df is not None
            and accession_number in self.patient_data_df.index
        ):
    ...
    def get_current_folder_details(self) -> dict | None:
        ...
        """
        if (
            not self.navigable_folders
            or self.current_folder_index < 0
            or self.current_folder_index >= len(self.navigable_folders)
        ):
            return None
        ...
        if metadata is None:  # Safety check; unlikely if _scan_patient_folders worked
            print(
                f"Warning: Metadata not found for {fingerprint_value(accession_number, 'accession')} in train.csv."
            )
            return {
    ...
    def move_to_next_folder(self) -> bool:
        ...
        if (
            not self.navigable_folders
            or self.current_folder_index >= len(self.navigable_folders) - 1
        ):
...
if __name__ == "__main__":
    ...
    print(f"Attempting to access the archive directory at: {redact_path(archive_path)}")
    ...
    try:
        ...
        print(
            f"Total navigable folders initially: {data_manager.get_total_navigable_folders()}"
        )
        ...
        print(
            f"Navigable folders after filter: {data_manager.get_total_navigable_folders()}"
        )

        current_details = data_manager.get_current_folder_details()
        if current_details:
            accession_token = fingerprint_value(
                current_details["accession_number"], "accession"
            )
            print(
            ...
            print(f"  PNGs: {_redact_values(current_details['png_files'], 'png')}")
        else:
        ...
        if data_manager.get_total_navigable_folders() > 1:
            ...
            if data_manager.move_to_next_folder():
                ...
                accession_token = fingerprint_value(
                    next_details["accession_number"], "accession"
                )
                print(
                    f"Next Folder Details ({data_manager.get_current_folder_index_display()}/{data_manager.get_total_navigable_folders()}): {accession_token}"
                )

            if data_manager.move_to_previous_folder():
                prev_details = data_manager.get_current_folder_details()
                accession_token = fingerprint_value(
                    prev_details["accession_number"], "accession"
                )
                print(
                    f"Previous Folder Details ({data_manager.get_current_folder_index_display()}/{data_manager.get_total_navigable_folders()}): {accession_token}"
                )
        ...
        if current_details_all:
            accession_token = fingerprint_value(
                current_details_all["accession_number"], "accession"
            )
            print(
                f"\nCurrent Folder Details ({data_manager.get_current_folder_index_display()}/{data_manager.get_total_navigable_folders()}): {accession_token}"
            )
