...
class DicomImageLoader:
    ...
    def load_dicom_data(self, dicom_path: str) -> tuple[np.ndarray | None, dict | None]:
        ...
        try:
            ...
            has_dicom_window = hasattr(ds, "WindowCenter") and hasattr(
                ds, "WindowWidth"
            )

            # Use centralized extraction function
            window_center, window_width, photometric_interpretation = (
                extract_window_parameters(ds, pixel_array)
            )
...
if __name__ == "__main__":
    ...
    if test_dicom_path and os.path.exists(test_dicom_path):
        ...
        if pixel_data_float is not None and view_params is not None:
            print(
                f"DICOM data loaded. Shape: {pixel_data_float.shape}, Type: {pixel_data_float.dtype}"
            )
            print(
                f"Raw data min/max (after rescale): {np.min(pixel_data_float):.2f}, {np.max(pixel_data_float):.2f}"
            )
