...
def parse_float_list(raw: Optional[str], expected_len: Optional[int] = None, name: str = "values") -> Optional[List[float]]:
    """Parse comma/space-separated floats, validating the expected length when provided."""
    if raw is None:
        return None
    text = str(raw).strip()
    if not text:
    ...
    try:
        ...
        raise ValueError(f"{name} deve conter apenas numeros. Recebido: {raw}") from exc
